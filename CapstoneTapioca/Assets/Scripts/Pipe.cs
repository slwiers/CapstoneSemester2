...
public class Pipe : MonoBehaviour
{
    ...
    public void Init(int pipe)
    {
        ...
        currentPipe.transform.localEulerAngles = new Vector3(0, 0, rotation * rotationMultiplier);
    ...
    public void UpdateFilled()
    {
        ...
        // Check if any connected pipe is filled
        List<Pipe> connected = ConnectedPipes();
        bool wasFilledBefore = IsFilled;
        
        foreach (var connectedPipe in connected)
        {
            if (connectedPipe != null && connectedPipe.IsFilled)
            {
                IsFilled = true;
                break;
            }
        ...
        // Only update sprites if state changed
        if (wasFilledBefore != IsFilled)
        {
            Debug.Log($"{gameObject.name} changed to IsFilled={IsFilled}");
        }
    ...
    public List<Pipe> ConnectedPipes()
    {
        ...
        {
            RaycastHit[] hit = Physics.RaycastAll(box.transform.position, Vector3.forward, 0.1f);
            for (int i = 0; i < hit.Length; i++)
            {
                // Guard against null colliders
                if (hit[i].collider == null) continue;

                // Try to get Pipe directly from the hit collider first
                Pipe pipe = hit[i].collider.GetComponent<Pipe>();
                
                // If not found, try parent hierarchy (but with safety)
                if (pipe == null && hit[i].collider.transform.parent != null)
                {
                    pipe = hit[i].collider.transform.parent.GetComponent<Pipe>();
                }
                
                if (pipe == null && hit[i].collider.transform.parent?.parent != null)
                {
                    pipe = hit[i].collider.transform.parent.parent.GetComponent<Pipe>();
                }

                // Only add valid pipes that aren't self
                if (pipe != null && pipe != this && !result.Contains(pipe))
                {
                    result.Add(pipe);
                    Debug.Log($"Connected pipe found: {pipe.gameObject.name}");
                }
            }
        }
