...
public class GameManager : MonoBehaviour
{
    ...
    private void CheckFill()
    {
        ...
        foreach (var filled in finished)
        {
            ...
            Debug.Log($"Pipe at {filled.transform.position} marked as filled");
        }
    ...
    private void CheckWin()
    {
        for (int i = 0; i < _level.Row; i++)
        {
            ...
            {
                Pipe pipe = pipes[i, j];
                // Only check pipes that are NOT empty (PipeType != 0)
                if (pipe.PipeType != 0 && !pipe.IsFilled)
                {
                    return;
                }
            }
    ...
    private IEnumerator GameFinished()
    {
        ...
        UnityEngine.SceneManagement.SceneManager.LoadScene("WaterRoom7");
    }
